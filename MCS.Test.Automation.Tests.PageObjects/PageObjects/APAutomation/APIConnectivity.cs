...
namespace MCS.Test.Automation.Common.Helpers
{
    public static class APIConnectivity
    {
        ...
        public static async Task<List<HttpResponseMessage>> SendAPIRequest(string url, string jsonPath)
        {
            try
            {
                return await APIResponse.GetResponse(url, jsonPath);
            }
            catch (Exception ex)
            {
                DriverContext.ExtentStepTest.Log(LogStatus.Info, "SendAPIRequest : Failed due error " + ex.Message);
                return null;
            }
            //return response;
        }
