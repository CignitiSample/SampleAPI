...
namespace MCS.Test.Automation.Common.Helpers
{
    ...
    public static class FilesHelper
    {
        ...
        /// <summary>
        /// This method is used to read the Docx file text
        /// </summary>
        /// <param name="wordFileName">Word file name</param>
        /// <param name="textToCheck">text ro search</param>
        /// <returns>true</returns>
        public static bool ExtractAndValidateTextFromDocx(string wordFileName, string textToCheck)
        {
            try
            {
                string result = string.Empty;
                ...
                using (WordprocessingDocument wordDocument =
                     WordprocessingDocument.Open(wordFileName, false))
                {
                    var header = wordDocument.MainDocumentPart.HeaderParts.First();
                    string text = header.Header.InnerText;
            ...
            {
                Logger.Error("File not found : " + wordFileName + ex.ToString());
                throw;
            }
        }

        public static DataSet ReadExcelFileReturnTable(string FilePath)
        {
            try
            {
                Thread.Sleep(3000);
            ...
            {
                Logger.Error("File not found : " + FilePath + ex.ToString());
                throw;
            }
        }
